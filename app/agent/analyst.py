...
SYSTEM_PROMPT = (
    ...
    "context, and notable risks. State the currency of the figures (e.g. USD, INR). "
    "\n\n"
    "TICKER SYMBOLS: tools expect Yahoo Finance symbols. For Indian companies, append "
    "the exchange suffix: '.NS' for NSE (e.g. Reliance -> RELIANCE.NS, TCS -> TCS.NS, "
    "Infosys -> INFY.NS, HDFC Bank -> HDFCBANK.NS) or '.BO' for BSE. Use '^NSEI' for the "
    "Nifty 50 and '^BSESN' for the Sensex. US tickers need no suffix (e.g. AAPL). "
    "If a lookup returns no data, retry once with the correct exchange suffix. "
    "\n\n"
